...
#> ℹ Lesson bootstrapped in 3.841728 secs
#> → Bootstrapping example lesson
...
#> ℹ Remote set up in 0.01421261 secs
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 3.88 seconds
tik <- Sys.time()
...
#> [md-outputs (root-commit) 660698a] Initializing md-outputs branch
#> Running git push sandpaper-local 'HEAD:md-outputs'
#> To /tmp/RtmpoKugO1/REMOTE-1cfe6d79eb4d
...
#> From /tmp/RtmpoKugO1/REMOTE-1cfe6d79eb4d
...
#>   /tmp/RtmpoKugO1/file1cfe54a9ffe9/lesson-example/site/built \
#>   sandpaper-local/md-outputs
#> Preparing worktree (resetting branch 'md-outputs'; was at 660698a)
#> branch 'md-outputs' set up to track 'sandpaper-local/md-outputs'.
#> HEAD is now at 660698a Initializing md-outputs branch
#> ::endgroup::
...
#> [gh-pages (root-commit) e365d9d] Initializing gh-pages branch
...
#>   /tmp/RtmpoKugO1/file1cfe54a9ffe9/lesson-example/site/docs \
#>   sandpaper-local/gh-pages
#> Preparing worktree (resetting branch 'gh-pages'; was at e365d9d)
#> branch 'gh-pages' set up to track 'sandpaper-local/gh-pages'.
#> HEAD is now at e365d9d Initializing gh-pages branch
#> ::endgroup::
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 0.41 seconds
tik <- Sys.time()
...
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 14.29 seconds
tik <- Sys.time()
cli::cli_h1("Clean Up")
#> 
#> ── Clean Up ────────────────────────────────────────────────────────────
cli::cli_alert_info("object db is an expression that evaluates to {.code {db}}")
#> ℹ object db is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/RtmpoKugO1/file1cfe54a9ffe9/lesson-example/site/built", "/tmp/RtmpoKugO1/file1cfe54a9ffe9/lesson-example")`
eval(db)
...
#> ℹ object ds is an expression that evaluates to `sandpaper:::github_worktree_remove("/tmp/RtmpoKugO1/file1cfe54a9ffe9/lesson-example/site/docs", "/tmp/RtmpoKugO1/file1cfe54a9ffe9/lesson-example")`
eval(ds)
#> Running git worktree remove --force \
#>   /tmp/RtmpoKugO1/file1cfe54a9ffe9/lesson-example/site/docs
#> $status
...
#> ℹ removing 'sandpaper-local' (/tmp/RtmpoKugO1/REMOTE-1cfe6d79eb4d)
#> /tmp/RtmpoKugO1/REMOTE-1cfe6d79eb4d
sandpaper:::reset_git_user(res)
# remove the test fixture and report
tryCatch(fs::dir_delete(res), error = function() FALSE)
tok <- Sys.time()
cli::cli_alert_info("Elapsed time: {round(tok - tik, 2)} seconds")
#> ℹ Elapsed time: 0.18 seconds
```
